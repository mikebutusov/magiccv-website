...
export function TestimonialBlock({ context = "bid manager, IT consulting, NL" }: { context?: string }) {
  return (
    <Container>
      <div className="grid items-center gap-8 rounded-3xl border border-dashed border-border bg-surface p-8 md:grid-cols-[auto_1fr] md:p-10">
        <PlaceholderBlock label={`{{TESTIMONIAL: ${context}}}`} aspect="aspect-square" className="w-24 md:w-28" />
        <div>
          <p className="font-display text-xl leading-snug text-ink-soft">
